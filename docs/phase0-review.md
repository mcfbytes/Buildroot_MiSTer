...
| #13 | — | `mt` is a MiSTer-only U-Boot command (added by `c0ed23f52e` in U-Boot fork). `fpgacheck` depends on it. | Verified in fork source | Informational for P5.1 (mainline U-Boot port must reimplement `mt` or rewrite `fpgacheck`) *(moot since ADR 0017: Phase 5 builds the fork, `mt` included)* |
