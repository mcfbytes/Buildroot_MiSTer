...
The variant builds end-to-end (`make rt`, **36 of the 37 shared + 4 beta-local**
carried patches — **40 entries; the one omission is `0047`, which 7.2 already has
in-tree, see §2**; three of the
...
the full **40-entry** series, 2026-08-17.
...
| **Patch 0031 (exFAT Samsung symlinks) on 7.x** | ❌ **BROKEN — found 2026-09-06**, not by this variant but by the DE25's aarch64 initramfs QEMU leg (`scripts/test-initramfs.sh --board de25nano`), which is the first thing ever to EXECUTE 0031 on a 7.x kernel: `exfat_symlink()` → `page_symlink()` → `a_ops->write_begin` is NULL on 7.x exFAT (iomap; no `write_begin`/`write_end`) → Oops, `pc: 0x0`, on the first symlink created. `linux-patches-beta/0031` is a symlink to the same file, so the RT kernel has the same crash on any `ln -s` on `/media/fat` (reading existing links is fine). The "applies at -F0" and "compiles" rows above are true and insufficient. Fix: a beta-local 7.x re-anchor that writes the link target without `page_symlink`; the aarch64 leg's `symlink` case is the acceptance test (ADR 0002 §8b, `docs/de25-nano-tasks.md` next-steps item 2). |
