...
| 25 | `0031-exfat-samsung-symlinks` | portable-as-is / shared | **included — BROKEN ON 7.x, fix owed** | Filesystem-format code; `inode_nohighmem()` is a no-op without HIGHMEM (normal on arm64). **Found 2026-09-06 by `scripts/test-initramfs.sh --board de25nano` (the first time this patch ran on a 7.x kernel anywhere):** `exfat_symlink()` writes the link target through `page_symlink()`, which calls `a_ops->write_begin` — and 7.x exFAT is iomap-based with no `write_begin`/`write_end`, so symlink CREATION is a NULL call and a kernel Oops (`pc: 0x0`). Applies cleanly at `-F0` and compiles, which is why the triage above missed it. Reading existing links (`page_get_link` → `read_folio`) is unaffected. Needs a 7.x re-anchor (write the target without `page_symlink`); the DE10's RT series shares this file and has the same bug (ADR 0002 §8b). |
