...
|---|---|---|---|
| **OpenSSL** | **1.1.1** (`libssl.so.1.1`) | **3.6.2** (`libssl.so.3`) | **OpenSSL 1.1.1 reached end-of-life on 2023-09-11** — stock ships a TLS library that has received **no** upstream fixes for ~2 years. This is the single strongest security argument. Major SONAME bump `1.1 → 3`. |
| **OpenSSH** | **8.6p1** | **10.2p1** | the network login surface; ~4 years and several release cycles of hardening |
...
SONAMEs mapped to a Buildroot 2026.02.3 package with its version and whether the
