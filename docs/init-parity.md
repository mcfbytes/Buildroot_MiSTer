...
Task: **P2.3**. Depends on P0.3 (`docs/stock-inventory/20250402/etc-configs.md`) and P2.1 (full
...
| `/etc/hosts` | **extended (issue #188)** | Stock's two lines byte-for-byte (`127.0.0.1 localhost`, `127.0.1.1 MiSTer`), plus the conventional `::1 localhost ip6-localhost ip6-loopback` and `ff02::1`/`ff02::2` lines, since `lo` keeps `::1` while IPv6 is otherwise off (`docs/kernel-config-deltas.md` D11). A user's `/media/fat/linux/hosts` still replaces the whole file on update. Buildroot's own finalize hook writes `127.0.1.1 buildroot` into `output/target/etc/hosts` **before** the overlay is copied (confirmed in the build log); the overlay copy runs later and wins — verified in the extracted image. |
