...
| **Mounting a NAS share** | `Scripts_MiSTer/cifs_mount.sh`, which works there — and refuses to run on any kernel ≥ 6.8, ours included: it requires an `fscache.ko` that Linux 6.8 folded into `netfs.ko` (making `CONFIG_FSCACHE` a `bool`, so it can never be a module again), and reports "The current Kernel doesn't support CIFS (SAMBA)" on a kernel whose CIFS is complete | **Scripts > mount_smb.sh** — a clean reimplementation that asks `/proc/filesystems` instead of guessing from module filenames. Reads your existing `cifs_mount.ini`, and its boot entry is guarded so it does not also fire on shutdown ([`docs/cifs-mount-fscache-probe.md`](docs/cifs-mount-fscache-probe.md)) |
