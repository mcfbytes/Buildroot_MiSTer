...
| **Image size** | 375 MiB, **13.6% free** ("93% full") — **not re-measured for Release 20260907**: the released artifacts are tarballs, not a formatted `linux.img`, and rebuilding one requires `create_img.sh` + root + `mkfs.ext4`, not attempted this pass. `rootfs.tar.bz2` itself grew only 82,617,954 → 82,661,550 bytes (+0.05%), but the *installed* tree (after `create_img.sh`'s module/firmware/addon overlay) grew from ~267.2 MiB to **294.4 MiB apparent content** (`docs/stock-inventory/20260907/disk-usage.md`) — so free space on a formatted 375 MiB image almost certainly shrank, but by how much is unmeasured | 512 MiB, **39.5% free** — 310 MiB used / 202 MiB free, temporary debug block included, measured by `./scripts/check-size-budget.sh output/images/linux.img` on the image built 2026-07-27 **with** this branch's full package set (the T5 utilities, 7-Zip, and the rtl8852cu driver all included). Floor is 15%. ⚠️ [`docs/size-budget.md`](docs/size-budget.md) still reports 60.6% free: it is a P3.3-vintage report and needs regenerating |
| **Rootfs build recipe** | **Not published.** `MiSTer-devel/Linux_Image_creator_MiSTer` exists, but it is an *assembler*, not a builder: `create_img.sh` runs `mkfs.ext4` and untars a prebuilt **82 MiB `rootfs.tar.bz2` committed to git**. Nothing public turns source into that tarball, so the image can be re-packed but its contents cannot be changed, patched or audited beyond `ls` — **still true for Release 20260907**, same `create_img.sh`, same shape | This repository |
