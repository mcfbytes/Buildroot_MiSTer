...
* it is *not* built by the kernel-only fragment stack (`package/linux-rt/linux-rt.mk`) / `make linux-rt`
...
`linux.config` is shared with the kernel-only stack (`configs/mister_de10nano_defconfig` names it), so the RT/beta
...
6. Rebuild:
   ...
   ```sh
   make mister_de10nano_defconfig    # re-resolve output/.config from the committed defconfig
   make all
   ```
...
`7.2.0-rc4` RT tree from a previous `make linux-rt`, 3.2 MiB), and this branch has only
`6.18.39` — no `make linux-rt` was run in this worktree. Comparing the images
