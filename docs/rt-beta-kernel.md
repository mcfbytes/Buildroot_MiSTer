...
| 29/31 *carried* patches apply to 7.2-rc3 at Buildroot's `patch -F0` (`linux-patches/`; the separate `linux-patches-upstream/` series is never applied to this variant — see §2) | ✅ verified through the real `linux-patch` stage (0015 + 0031 re-anchored; the old "28/31" figure was measured at `patch`'s default fuzz 2, which Buildroot forbids, and wrongly counted 0015 as upstreamed) |
