...
> **SUPERSEDED for onboarding, 2026-08-01 — the analysis below is still correct, but it is
> no longer the whole mechanism.** This section reasons about how to *win* the multi-db
> race for the single Linux slot. The project no longer enters that race: onboarding now
> also sets `update_linux = false` in the `[MiSTer]` section of the base
...
### 11.2 — Our project's entry (shape; live values are generated by `scripts/gen-db-json.py`)

The live document is at <https://mcfbytes.github.io/Buildroot_MiSTer/db.json> — fetch it
rather than transcribing the placeholders below, which are kept only as the shape
reference.
...
5. **`files/linux/` in our archive carries the full auxiliary payload** the rsync step
   ...
   `mt32-rom-data/`, `soundfonts/`, `zImage_dtb`, `uboot.img`, and — since ADR 0023 — our own
   statically-linked `7za`, §4) — a missing file here isn't
