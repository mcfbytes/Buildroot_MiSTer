...
| `configs/mister_rt.fragment` | Buildroot-config delta, layered on the `de10nano-kernel` fragment stack by `make rt` (kernel version → the 7.2 line, currently **`7.2.2`** — a plain release since 2026-08-17, so Buildroot's `-rc` cgit-snapshot path in `linux/linux.mk` no longer applies and it fetches the ordinary `linux-7.2.tar.xz` from the `v7.x` mirror directory; beta patch dir; kernel-config fragment). Merged onto `mister_kernel_defconfig` via `merge_config.sh`. |
