...
**7.2-rc5**, complete **31/31** patch series) and **boots and runs MiSTer on a
...
| **31/31** *carried* patches apply to 7.2-rc5 at Buildroot's `patch -F0` (`linux-patches/`; the separate `linux-patches-upstream/` series is never applied to this variant — see §2) | ✅ **re-verified on the rc4 → rc5 bump (2026-07-28)** through Buildroot's own `apply-patches.sh` against a pristine 7.2-rc5 tarball: 31/31 applied, exit 0, zero hunks taking fuzz (70 hunks land at an offset, which `-F0` permits). No re-anchor was needed for rc5, so all four beta-local copies (0001, 0015, 0030, 0037) carry over unchanged. The series has dropped nothing since 2026-07-20, when `0037` and `0030` were re-anchored and re-included — the 7.x kernel diverges from the 6.18 kernel only where an upstream API forced a re-anchor, never by omission. Earlier figures on this row (29/29, and before that a bogus "28/31" measured at `patch`'s default fuzz 2, which Buildroot forbids) predate those two re-anchors |
