...
| `e155f6a2f`, `2799f8b94`, `b00a72159` | NSO NES/SNES, N64, Mega Drive | **`94f18bb19945`** *HID: nintendo: add support for nso controllers* (2023-12-04) | 6.18 `hid-ids.h:1066-1068`: `SNESCON 0x2017`, `GENCON 0x201e`, `N64CON 0x2019`. **Device-ID table is identical to the fork's.** **2026-07-15:** two behavioral divergences found (kernel-recon): NES/Famicom **A/B are swapped** vs stock → restored via `0034-hid-nintendo-nes-famicom-stock-ab-mapping.patch`; `b00a72159`'s BT product-ID forcing (MD pad reports SNES PID) is not upstream. **CORRECTED 2026-07-24 → carried, `0038-hid-nintendo-nso-genesis-bt-pid.patch`.** The earlier "known low-impact limitation … not carried" ruling was wrong on both counts: it is a **stock-parity gap** (stock forces the PID at `MiSTer-v5.15:hid-nintendo.c:1761-1765`; `grep -c 'hdev->product = '` on v6.18.39 = **0**), and the impact is not low — *because* SDL gamecontrollerdb keys off raw VID:PID, the bogus PID makes a Bluetooth Genesis pad match the **NSO SNES** row and come out mismapped. Missed because stock names the device `MDCON`/`JOYCON_CTLR_TYPE_MD` where mainline names the same PID/value `GENCON`/`JOYCON_CTLR_TYPE_GEN`. Found by tree-diffing our export against upstream's own 6.18 port — see `docs/kernel-recon/fork-sync-2026-07.md` §8.1. |
...
|---|---|---|---|
| `4ddd8ec3d`, `5a7965488`, `c708f2222`, `e2eb39e6f`, `8270e78f4`, `d776ddb4e`, `d5beb5aa6` | Add/refresh xone (Xbox wireless adapter, GIP) — 19 files, `drivers/hid/xone/**`, `CONFIG_JOYSTICK_XONE=m` | vendored from **`medusalix/xone`** (MIT/GPL); MiSTer-local additions: rumble fix, Elite-2 paddles backport, per-PID firmware, sysfs software pairing | **✅ re-sourced (P3.2)** → `package/xone`, from **`dlundqvist/xone`** (the actively-maintained fork — `medusalix/xone`, the original this was vendored from, is explicitly "in maintenance mode" per its own README). **All 4 of MiSTer's local deltas turned out to be unnecessary as separate patches**: checked directly in the pinned source, not assumed — `driver/gamepad.c` already carries rumble handling and a `PaddleCapability` enum with distinct `PADDLE_ELITE`/`PADDLE_ELITE2_4X`/`PADDLE_ELITE2_510` firmware-version cases (a *more* granular Elite-2 paddle implementation than a simple backport), `transport/dongle.c` already builds per-PID firmware filenames (`sprintf(fwname, "xone_dongle_%04x.bin", fw_product)`), and already exposes a mutex-guarded sysfs `pairing` attribute (`xone_dongle_pairing_handler`) — the actively-maintained fork independently grew equivalent functionality to what MiSTer's maintainer patched onto the older base. Zero compat patches needed for 6.18 either (see `package/xone/xone.mk`). Firmware redistribution: **decided** — `docs/decisions/0003-xone-firmware.md` (Accepted), `package/xow-firmware` + `package/cabextract`. **Not upstream in mainline** (`grep -rl gip_driver drivers/` in 6.18 → nothing) |
...
| **Q9** | ~~DualSense (4 class-C commits)~~ **RESOLVED (2026-07-15)** at source level, no HW needed: the grouping was wrong (see corrected §3.4 rows) — `:player_id` carried (`0033`), lightbar superseded-better upstream, mute carried (`0037`). **"mute open (cosmetic)" was wrong — corrected 2026-07-20:** `BTN_Z` (`0x135`) shifts every higher gamepad button one index up in the `EV_KEY` bitmap, and Main_MiSTer resolves gamecontrollerdb `bN` indices off that bitmap; the shipped `platform:MiSTer` PS5 rows encode the BTN_Z-present layout (`guide:b11`). Dropping `0037` slides the whole DualSense map by one. On-device sanity check of `0033` still advisable at P3.13. | ~~LOW~~ done | ~~P3.13~~ |
...
| `0040-hid-nintendo-imu-name-suffix` | `a6b7e3666` (Sorgelig) | **RETIRED 2026-09-12 — slot intentionally empty.** Was: Main_MiSTer excludes the IMU input device with `strstr(name, " IMU")` and that string test is its only hook; mainline's `"%s (IMU)"` does not match, so the IMU node is opened as a phantom controller. Offered upstream as Linux-Kernel_MiSTer PR #96 alongside a userspace alternative; the maintainer closed the kernel PR ("better to follow mainline to be compatible with upstream kernel source") and merged Main_MiSTer #1307, which also matches `"(IMU)"`. Shipped in Main_MiSTer Release 20260912, so the kernel no longer needs to carry the name. The patch header's second consumer, SDL2's `SDL_strstr(name, " IMU")` filter, is *not* covered: an SDL2 program on this image now enumerates a Switch pad's `(IMU)` node as a second game controller — the same thing stock's own 6.18 kernel does, and nothing shipped uses SDL2's controller layer for it. |
| `0041-hid-nintendo-stock-led-classdev-names` | `60821059c` (Sorgelig) | **RETIRED 2026-09-12 — slot intentionally empty.** Was: stock `player1`-`player4`/`home` LED classdev names — Main_MiSTer opens these by exact path and treats a failed `fopen()` as a silent no-op, so under mainline's names every Switch controller loses userspace LED control (same origin commit as `0035`). Offered upstream as Linux-Kernel_MiSTer PR #97; closed ("we will use mainline version to be compatible with further updates") in favour of Main_MiSTer #1308, which falls back to `:green:player-1`..`-4` and `:blue:player-5` when the 5.15 paths are absent. Shipped in Release 20260912. `0035` (registration non-fatal) and `0032`'s `:combo` node are unaffected — `:combo` never had a mainline name to fall back to. |
...
### `0054`–`0060` — original dwc2 host work, added 2026-09-25 (#205)
...
sent upstream yet (owner decision, after rig soak).
