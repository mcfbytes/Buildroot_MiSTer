...
| **Mounting a NAS share** | `Scripts_MiSTer/cifs_mount.sh` — which refused to run on any kernel ≥ 6.8 until we [fixed it upstream](https://github.com/MiSTer-devel/Scripts_MiSTer/pull/141) (merged 2026-08-17). Nothing distributes that file, though, so the fix reaches a card only if its owner re-downloads it by hand | **Scripts > mount_smb.sh** — ships with the image, so an SMB mount works with no hunting on GitHub. Reads your existing `cifs_mount.ini`, and its boot entry is guarded on `$1` so it does not also fire during shutdown — still an open bug upstream ([`docs/cifs-mount-fscache-probe.md`](docs/cifs-mount-fscache-probe.md)) |
