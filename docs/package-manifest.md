...
| `libtorrent.so.21` | `libtorrent.so.21.0.0` | `BR2_PACKAGE_LIBTORRENT` | 0.15.3 | **YES (likely)** | rakshasa's libtorrent (rtorrent's library, distinct from libtorrent-rasterbar); SONAME has moved past 21 in this version range per Debian's experimental packaging (`libtorrent27`). **Recommend dropping `rtorrent`/`libtorrent` entirely (see Drop list) — nothing MiSTer-side uses it**, so the exact soname doesn't matter. **Still dropped, but the premise is no longer true (2026-09-21, issue #186):** this image now ships a BitTorrent client again, `transmission` (§5's row, `docs/bittorrent.md`). It does **not** restore this SONAME — transmission links its own static `libtransmission` and shares no code with rakshasa's libtorrent — so anything that `dlopen`ed `libtorrent.so.21` on stock is still broken here, which remains a hypothetical: nothing in the ecosystem does |
...
| Realtek out-of-tree WiFi — **stale row, kept for history; see the note below** (11ac: `8812au`, `8821au`) | kernel modules, class E | none (by design — morrownr forks) | **Not this task's gap** — owned by P3.1/v9 as Buildroot `kernel-module` packages sourced from morrownr. v9 moved `8188eu`/`rtl8188fu` (→ in-kernel `rtl8xxxu`), `8821cu` (→ `rtw88_8821cu`) and `88x2bu` (→ `rtw88_8822bu`) to mainline; PR #35 later moved `8814au` (→ in-kernel `rtw88_8814au`) the same way, ~~leaving only `8812au` and `8821au` out-of-tree (no mainline USB driver)~~. **Superseded twice**: v10 moved `8812au`/`8821au` to mainline too (`rtw88_88xxa` core, 6.13), taking the out-of-tree count to zero; v10.2 then added **one** new out-of-tree package for a chip that was never in this row at all — `rtl8852cu-morrownr` (RTL8852CU/RTL8832CU Wi-Fi 6E), because mainline's `rtw89` HAL for that chip has no USB bus file, only a PCI one this board can't reach. ~~Current state: **one** out-of-tree WiFi kernel-module package, not the two named in this row's title.~~ **Superseded a third time (2026-09-10):** `package/aic8800` was added for the AICSemi AIC8800 family (Wi-Fi 6 + BT, driver **and** ~6.6 MiB of firmware, from `radxa-pkg/aic8800`) — mainline has no aic8800 driver over any bus — and the **seven deselected packages named in this row were deleted outright** (`rtl8188eu-aircrack-ng`, `rtl8188fu`, `rtl8812au`, `rtl8814au-morrownr`, `rtl8821au-morrownr`, `rtl8821cu-morrownr`, `rtl88x2bu`), because a package the image never builds still costs a Renovate PR stream, a hash-sync entry and a CI path filter. Current state: **two** out-of-tree WiFi kernel-module packages (`rtl8852cu-morrownr`, `aic8800`) and **zero** sourced-but-deselected ones. See [ADR 0016](decisions/0016-mainline-first-wifi-drivers.md) (its v10.2 update has the gap analysis, its 2026-09-10 update the two changes above) and `docs/wifi-parity.md` §8, §10.1, §11, §12 | P3.1/v9 → 2026-09-10 |
...
| `rtorrent` + `libtorrent` (rakshasa) | binary + lib | nothing in MiSTer's ecosystem uses a BitTorrent client on-device; the SONAME has also drifted past stock's `.so.21` in this version range (Debian ships `libtorrent27` for 0.15.7) so it isn't even a clean 1:1 carry-forward | **rtorrent itself: still dropped. The capability: RESTORED (2026-09-21, issue #186) by a different package.** This row is the only place the manifest records that **stock ships a BitTorrent client** — `usr/bin/rtorrent` + `libtorrent.so.21`, present in both the 20250402 and 20260907 stock inventories — so when issue #186 argued for one it was arguing to restore a stock capability, not to add a new one, and this row's *use-case* premise is what it overturned: freely redistributable preservation and homebrew sets are published over BitTorrent and nowhere else, and without a client the board needs a PC in the middle. The SONAME half of the reasoning stands and is why the answer is **not** to un-drop rtorrent. Neither is the deciding factor: several boards on one LAN should fetch a set once and get it from each other afterwards, which is **Local Peer Discovery**, and rakshasa's libtorrent 0.15.3 has none — a full `src/dht/` but no LPD at all; the standard group `239.192.152.143` appears nowhere in its tree. `BR2_PACKAGE_TRANSMISSION` + `_DAEMON` have LPD, DHT, PEX, µTP and per-file selection, are upstream-packaged, and ship **off by default** behind a card directory (`docs/buildroot-config.md` §5.10, `docs/bittorrent.md`, ADR 0031's 2026-09-21 amendment). `transmission-cli` is deliberately not built — static `libtransmission` means a second full copy of the engine, for the one binary with no file-selection options at all |
...
> (`configs/mister_de10nano_defconfig`), and `_SSL`/`_ZLIB` are **hard blockers**
