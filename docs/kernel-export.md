...
| Step | Result |
|---|---|
| **Export script bug found and fixed** | It read `BR2_PACKAGE_*=y` from `configs/mister_de10nano_defconfig` only; the 2026-09 fragment split moved those lines to `de10nano-image.fragment`, so the script's own "zero kernel-module packages" guard would have **aborted every export since**. It now resolves the DE10 stack from `configs/mister_*_defconfig` via `scripts/lib/config-stacks.sh` and reads all four fragments, last-wins |
