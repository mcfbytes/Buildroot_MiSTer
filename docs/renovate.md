...
| itsalive commit-SHA pin (SD-card installer HDMI splash tool, ADR 0020 §9; labeled `tool-pin` + `needs-manual-hash`) | `package/itsalive/itsalive.mk` | `customManagers` regex, `git-refs` datasource tracking `mcfbytes/ItsAlive_MiSTer`'s `master` HEAD via `currentDigest` (the project cuts no releases) | `package/itsalive/itsalive.hash` — **not auto-refreshed.** itsalive is a `cargo-package`: Buildroot vendors the crate graph and hashes the repacked `itsalive-<sha>-cargoN.tar.gz`, so the generic loop's `curl \| sha256sum` would write the pre-vendoring archive's hash (a wrong value that looks right) and the package is deliberately in **neither** `HASH_SYNC_PACKAGES` nor the workflow's `paths:` filter. Unlike azcopy there is no rebuild case for it yet (a cargo analogue of case 7 would be the fix), so a bump PR **fails closed** at `lint.yml`'s `itsalive version/hash pin consistency` step until a human runs the recipe in the `.hash` — `needs-manual-hash` is back in use for exactly this and means what it meant on azcopy before 2026-08-28 |
...
That is **20 customManagers entries** (Buildroot, the 6.18 kernel, the RT/beta
