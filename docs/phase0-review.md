...
| #13 | — | `mt` is a MiSTer-only U-Boot command (added by `c0ed23f52e` in U-Boot fork). `fpgacheck` depends on it. | Verified in fork source | Informational for the former P5.1, the mainline U-Boot port (it would have had to reimplement `mt` or rewrite `fpgacheck`) *(was moot under ADR 0017, which built the fork; **live again under ADR 0024**, which builds mainline — resolved, not by reimplementing `mt`, but with `itest.l *<addr> == <val>`, verified by execution. See `docs/uboot-mainline-port.md` §3.4)* |
