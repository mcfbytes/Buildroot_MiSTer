...
| Realtek out-of-tree WiFi (`8188eu`, `rtl8188fu`, `8812au`, `8821au`, `8821cu`, `88x2bu`) | kernel modules, class E | none (by design — morrownr forks) | **Not this task's gap** — already owned by P3.1 as Buildroot `kernel-module` packages sourced from morrownr, per PLAN §4.1 class E. Listed here only to confirm P0.7 isn't duplicating that work | P3.1 |
| `xone` (Xbox Wireless) | kernel modules, class D | none (out-of-tree upstream project) | **Not this task's gap** — owned by P3.2 | P3.2 |
...
| `vim`, `zsh`, `joe`, `mc` (Midnight Commander), `screen`, `gdb` | ~15 MiB combined (vim 9.13, zsh 4.22, joe 0.3+0.28, mc 1.1+1.1, screen 0.27, gdb 0.4) | interactive power-user shell tools with no MiSTer-specific role; BusyBox's `vi`/`ash` cover the minimum needed for on-device editing/debugging | recommend dropping the heavy ones (vim, zsh) and keeping the light ones (joe, mc) if the community expects them — **P2.7's call**, flagged here as budget, not mandated |
| `/usr/share/zoneinfo` full tzdata, `/usr/share/consolefonts`+`/usr/share/keymaps` | ~2.6 MiB (zoneinfo 1.57, fonts+keymaps ~1) | stock's `/timezone` is fixed to `Etc/UTC` anyway; console fonts/keymaps beyond the one stock actually uses are unused | Buildroot's `tzdata`, `kbd`, and console-font packages all support trimming to a subset — but international users likely rely on the full zoneinfo set for `TZ=`, so this is a smaller/lower-confidence trim than the ones above |
