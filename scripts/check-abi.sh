...
if [ -z "$MODROOT" ]; then
	...
	KVERS="$(find "$MODROOT" -mindepth 1 -maxdepth 1 -type d -printf '%f\n' | LC_ALL=C sort)"
	if [ -z "$KVERS" ]; then
		fail "A-25 modules.alias" "no kernel-version directory under $MODROOT/ -- no module tree was installed at all (§13.1 A-25)"
	else
		a25_bad=""
		for kv in $KVERS; do
			[ -s "$MODROOT/$kv/modules.alias" ] || a25_bad="$a25_bad $kv"
		done
		if [ -z "$a25_bad" ]; then
			pass "A-25 modules.alias regenerated by depmod (kernel(s): $(printf '%s' "$KVERS" | tr '\n' ' ' | sed 's/ $//'))"
		else
			fail "A-25 modules.alias" "missing or empty for kernel tree(s):$a25_bad under $MODROOT/ -- depmod did not run at image build for them (§13.1 A-25)"
		fi
	fi
fi
